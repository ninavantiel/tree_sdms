...
if __name__ == '__main__':
    ...
    median_area_forest_1981_2010 = sdms.map(lambda sdm: mask_sdm(sdm.select('covariates_1981_2010')).multiply(
        sdms_forest_area_lat_elev_fc.filter(ee.Filter.eq('species', sdm.get('system:index'))).first().getNumber('area_1981_2010').divide(1e12)
    ).toInt64()).median()
