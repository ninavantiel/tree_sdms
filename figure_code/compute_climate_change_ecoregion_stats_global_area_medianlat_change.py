...
def compute_species_stats_ecoregion(ecoid):    
    ...
    sdms_eco_area_medianlat = sdms_area_medianlat.filter(ee.Filter.inList('species', sdms_eco.aggregate_array('system:index'))).map(lambda f: f.set({
        'area_rel_change': ee.Algorithms.If(f.getNumber('area_1981_2010').neq(0), f.getNumber('area_2071_2100_ssp585').divide(f.getNumber('area_1981_2010')), -999), 'medianlat_shift': ee.Algorithms.If(
            f.getNumber('medianlat_1981_2010').neq(-999).Or(f.getNumber('medianlat_2071_2100_ssp585').neq(-999)), 
    ...
    print(sdms_eco_area_medianlat.filter(ee.Filter.eq('area_1981_2010',0)).first().toDictionary().getInfo())
    print(sdms_eco_area_medianlat.filter(ee.Filter.eq('area_2071_2100_ssp585',0)).first().toDictionary().getInfo())
    ...
    print(
        ...
        sdms_eco_area_medianlat.filter(ee.Filter.neq('area_1981_2010', 0)).aggregate_array('area_1981_2010').reduce(ee.Reducer.median()).getInfo(),
    ...
    print(
        ...
        sdms_eco_area_medianlat.filter(ee.Filter.neq('area_2071_2100_ssp585', 0)).aggregate_array('area_2071_2100_ssp585').reduce(ee.Reducer.median()).getInfo(),
    ...
    print(
        ...
        sdms_eco_area_medianlat.filter(ee.Filter.neq('medianlat_1918_2010', -999)).aggregate_array('medianlat_1981_2010').reduce(ee.Reducer.median()).getInfo()
    )

    print(
        sdms_eco_area_medianlat.aggregate_array('medianlat_2071_2100_ssp585').reduce(ee.Reducer.median()).getInfo(),
        sdms_eco_area_medianlat.filter(ee.Filter.neq('medianlat_2071_2100_ssp585', -999)).aggregate_array('medianlat_2071_2100_ssp585').reduce(ee.Reducer.median()).getInfo()
    )
    ...
    print(ee.Feature(None, ecoregions.filter(ee.Filter.eq('ECO_ID', ecoid)).first().toDictionary(['BIOME_NAME','BIOME_NUM','ECO_ID','ECO_NAME','REALM']).combine({
        'ECO_AREA': eco_area,
...
if __name__ == '__main__':
    compute_species_stats_ecoregion(369)
